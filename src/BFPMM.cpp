...
//' @export
// [[Rcpp::export]]
Rcpp::List BFOC_U_MTT(const arma::field<arma::vec>& y_obs,
                      const arma::field<arma::vec>& t_obs,
                      const double& n_funct,
                      const int& K,
                      const int& P,
  ...
  for(int i = 0; i < tot_mcmc_iters; i++){
    ...
    if((i % n_temp_trans) == 0 && i > 0){
      ...
      for(int j = 0; j < n_funct; j++){
        y_star_TT(j,0).row(0) = y_star(j,0).row(i % r_stored_iters);
      }
...
Rcpp::List BFPMM(const arma::field<arma::vec>& y_obs,
  ...
  for(int i = 0; i < tot_mcmc_iters; i++){
    updateZ_PM(y_obs, y_star, B_obs, B_star,Phi((i % r_stored_iters),0),
    ...
    updatePhi(y_obs, y_star, B_obs, B_star, nu.slice((i % r_stored_iters)),
    ...
    updateNu(y_obs, y_star, B_obs, B_star, tau.row((i % r_stored_iters)).t(),
    ...
    updateChi(y_obs, y_star, B_obs, B_star, Phi((i % r_stored_iters),0),
              nu.slice((i % r_stored_iters)), Z.slice((i % r_stored_iters)),
              sigma((i % r_stored_iters)), (i % r_stored_iters), r_stored_iters,
              chi);

    updateYStar(B_star, nu.slice((i % r_stored_iters)), Phi((i % r_stored_iters),0),
                Z.slice((i % r_stored_iters)), chi.slice((i % r_stored_iters)),
                sigma((i % r_stored_iters)), (i % r_stored_iters), r_stored_iters,
                y_star);
...
Rcpp::List BFPMM_Theta(const arma::field<arma::vec>& y_obs,
  ...
  for(int i = 0; i < tot_mcmc_iters; i++){
    ...
    updatePhi(y_obs, y_star, B_obs, B_star, nu.slice((i)),
              gamma((i),0), tilde_tau,
              Z.slice((i)), chi.slice((i)),
              sigma((i)), (i),
              tot_mcmc_iters, m_1, M_1, Phi);
    ...
    updateChi(y_obs, y_star, B_obs, B_star, Phi((i),0),
