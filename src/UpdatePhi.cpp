    ...
    for(int m = 0; m < Phi(iter,0).n_slices; m++){
      ...
      for(int i = 0; i < Z.n_rows; i++){
        for(int l = 0; l < y_obs(i,0).n_elem; l++){
          ph = Z(i,j) * (y_obs(i,0)(l) - arma::dot(nu.row(j),
                         B_obs(i,0).row(l)));
          ...
          for(int k = 0; k < nu.n_rows; k++){
            ...
            if(k != j){
              ph = ph - Z(i,k) * arma::dot(nu.row(j), B_obs(i,0).row(l));
            }
          }
          m_1 = m_1 +  chi(i,m) * B_obs(i,0).row(l).t();
        }

        // Check to see if there are unobserved time points of interest
        if(B_star(i,0).n_elem > 0){
          for(int l = 0; l < y_obs(i,0).n_elem; l++){
            ph = Z(i,j) * (y_star(i,0)(l) - arma::dot(nu.row(j),
                           B_star(i,0).row(l)));
            ...
            for(int k = 0; k < nu.n_rows; k++){
              ...
              if(k != j){
                ph = ph - Z(i,k) * arma::dot(nu.row(j), B_star(i,0).row(l));
              }
            }
            m_1 = m_1 +  chi(i,m) * B_star(i,0).row(l).t();
          }
        }
      ...
      //generate new sample
      Phi(iter,0).slice(m).row(j) = Rmvnormal(M_1 * m_1, M_1);
    }
  }
}
