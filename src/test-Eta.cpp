...
// Tests updating Eta under the functional model
//
...
// Tests updating Eta under the tempered functional model
//
...
// Tests updating Eta under the multivariate model
//
...
// Tests updating Eta under the multivariate tempered model
//
arma::field<arma::cube> TestUpdateEtaTemperedMV(){
...
context("Unit tests for Eta parameters") {
  ...
  test_that("Sampler for Eta parameters for the tempered multivariate model"){
    Rcpp::Environment base_env("package:base");
    Rcpp::Function set_seed_r = base_env["set.seed"];
    set_seed_r(1);
    arma::field<arma::cube> x = TestUpdateEtaTemperedMV();
    arma::cube est = x(0,0);
    arma::cube truth = x(1,0);
    bool similar = true;
    for(int i = 0; i < est.n_rows; i++){
