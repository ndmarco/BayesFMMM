             ...
             arma::cube& Z)
{
  ...
  for(int i = 0; i < Z.slice(iter).n_rows; i++)
  {
    ...
    {
      z_lpdf = lpdf_z(f_obs(i,0), S_obs(i,0),
                      phi.slice(Map.at(get_ind(Z.slice(iter).row(i).t()))), nu,
                      pi, Z.slice(iter), i, mean_ph_obs(i,0));
      z_new_lpdf = lpdf_z(f_obs(i,0), S_obs(i,0),
                          phi.slice(Map.at(get_ind(Z_ph.row(i).t()))), nu,
             ...
             arma::cube& Z)
{
  ...
  for(int i = 0; i < Z.slice(iter).n_rows; i++)
  {
    ...
    if(arma::accu(Z_ph.row(i)) > 0)
    {
      z_lpdf = lpdf_z(f_obs(i,0), S_obs(i,0), phi, nu, pi, Z.slice(iter), i,
                      mean_ph_obs(i,0));
      z_new_lpdf = lpdf_z(f_obs(i,0), S_obs(i,0), phi, nu, pi, Z_ph, i,
                          mean_ph_obs(i,0));
      acceptance_prob = z_new_lpdf - z_lpdf;
      rand_unif_var = R::runif(0,1);
