               ...
               arma::mat& M)
{
  ...
  for(int j = 0; j < Z.n_cols; j++)
  {
    M = M + Z(i,j) * arma::pinv(S_obs(i,0) * phi *
      phi.t() * S_obs(i,0).t(), arma::datum::eps);
  }
  M = arma::pinv(M, arma::datum::eps);
}
              ...
              arma::field<arma::mat>& M)
{
  for(int i = 0; i < Z.n_rows; i++)
  {
    ...
    for(int j = 0; j < Z.n_cols; j++)
    {
      M(i,0) = M(i,0) + Z(i,j) * arma::pinv(S_obs(i,0) * phi *
        phi.t() * S_obs(i,0).t(), arma::datum::eps);
    }
    arma::pinv(M(i,0), M(i,0), arma::datum::eps);
  }
                ...
                arma::vec& m)
{
  ...
  {
    m = m + Z(i,j) * arma::pinv(S_obs(i,0) * phi *
      phi.t() * S_obs(i,0).t(), arma::datum::eps)* S_obs(i,0) * nu.col(j);
  }
}
                     ...
                     arma::mat& tilde_M)
{
  int n1 = S_obs(i,0).n_rows;
  int n2 = S_star(i,0).n_rows;
  mp_inv.submat(0, 0, n1 - 1, n1 - 1) = S_obs(i,0) * phi * phi.t() * S_obs(i,0).t();
  mp_inv.submat(0, n1, n1 - 1, n1 + n2 - 1) =  S_obs(i,0) * phi * phi.t() * S_star(i,0).t();
  mp_inv.submat(n1, 0, n1 + n2 - 1, n1 - 1) = mp_inv.submat(0, n1, n1 - 1 , n1 + n2 - 1).t();
  mp_inv.submat(n1, n1, n1 + n2 - 1, n1 + n2 - 1) = S_star(i,0) * phi * phi.t() * S_star(i,0).t();
                    ...
                    arma::field<arma::mat>& tilde_M)
{
  ...
  for(int i = 0; i < Z.n_rows; i++)
  {
    n1 = S_obs(i,0).n_rows;
    n2 = S_star(i,0).n_rows;
    mp_inv(i,0).submat(0, 0, n1 - 1, n1 - 1) = S_obs(i,0) * phi * phi.t() * S_obs(i,0).t();
    mp_inv(i,0).submat(0, n1, n1 - 1, n1 + n2 - 1) =  S_obs(i,0) * phi * phi.t() * S_star(i,0).t();
    mp_inv(i,0).submat(n1, 0, n1 + n2 - 1, n1 - 1) = mp_inv(i,0).submat(0, n1, n1 - 1 , n1 + n2 - 1).t();
    mp_inv(i,0).submat(n1, n1, n1 + n2 - 1, n1 + n2 - 1) = S_star(i,0) * phi * phi.t() * S_star(i,0).t();
                     ...
                     arma::vec& tilde_m)
{
  ...
  for(int j = 0; j <  Z.n_cols; j++)
  {
    tilde_m = (mp_inv.submat(n1, 0, n1 + n2 - 1, n1 - 1) * (
      S_obs(i,0) * nu.col(j) - f_obs) + mp_inv.submat(n1, n1, n1 + n2 - 1, n1 + n2 - 1) *
        S_star(i,0) * nu.col(j));
  }
}
