...
Rcpp::List TestUpdateZ()
{
  ...
  for(int i = 0; i < 100; i++)
  {
    ...
    B_star(i,0) =  bspline_mat.submat(t_obs.n_elem, 0, t_obs.n_elem + t_star.n_elem - 1, 7);
  }
  ...
  for(int j = 0; j < 100; j++){
    ...
    y_obs(j, 0) = Rmvnormal(B_obs(j, 0) * mean, sigma_sq * arma::eye(B_obs(j,0).n_rows, B_obs(j,0).n_rows));
    y_star(j, 0).row(0) = Rmvnormal(B_star(j, 0) * mean, sigma_sq * arma::eye(B_star(j,0).n_rows, B_star(j,0).n_rows)).t();
    for(int i = 1; i < 100; i++){
...
{
  arma::field<arma::mat> B_obs(100,1);
  B_obs(0,0) = arma::zeros(4, 4);
  Rcpp::List mod = Rcpp::List::create(Rcpp::Named("elements 0", B_obs(0,0).n_elem),
                                      Rcpp::Named("elements 1", B_obs(1,0).n_elem));
  return mod;
}
