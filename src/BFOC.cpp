...
Rcpp::List BFOC_U_TT(const arma::field<arma::vec>& y_obs,
  ...
  double geom_mult = std::pow(beta_N_t, 1/N_t);
  for(int i = 1; i < N_t; i++){
  ...
  for(int i = 0; i < tot_mcmc_iters; i++){
    ...
    for(int l = 1; l < ((2 * N_t) + 1); l++){
      ...
      updatePhiTempered(beta_ladder(temp_ind), y_obs, y_star_TT, B_obs, B_star,
                        ...
                        chi_TT.slice(l), sigma(l), l, (2 * N_t) + 1, m_1, M_1,
                        Phi_TT);
      ...
      updateNuTempered(beta_ladder(temp_ind), y_obs, y_star_TT, B_obs, B_star,
                       ...
                       b_1, B_1, nu_TT);
      updateTau(alpha, beta, nu.slice(l), l, (2 * N_t) + 1, P_mat, tau_TT);
    ...
    if(logu < logA){
      ...
      chi_TT.slice(i % r_stored_iters) = chi_TT.slice(2 * N_t);
    ...
    nu.slice(i % r_stored_iters) = nu.slice((i+1) % r_stored_iters);
    chi_TT.slice(i % r_stored_iters) = chi_TT.slice((i+1) % r_stored_iters);
    pi.col(i % r_stored_iters) = pi.col((i+1) % r_stored_iters);
    sigma(i % r_stored_iters) = sigma((i+1) % r_stored_iters);
    Z.slice(i % r_stored_iters) = Z.slice((i+1) % r_stored_iters);
    delta.col(i % r_stored_iters) = delta.col((i+1) % r_stored_iters);
 Phi((i+1) % r_stored_iters,0);
    A.row(i % r_stored_iters) = A.row((i+1) % r_stored_iters);
    tau.row(i % r_stored_iters) = tau.row((i+1) % r_stored_iters);
    Phi(i % r_stored_iters,0) = Phi((i+1) % r_stored_iters, 0);
    ...
    if(((i+1) % 20) == 0){
      Rcpp::Rcout << "Iteration: " << i+1 << "\n";
      Rcpp::Rcout << "Accpetance Probability: " << accept_num / (i+1) << "\n";
      Rcpp::Rcout << "Log-likelihood: " << arma::mean(loglik.subvec((i % r_stored_iters)-19, (i % r_stored_iters))) << "\n";
      Rcpp::checkUserInterrupt();
    }
    if(((i+1) % r_stored_iters) == 0 && i > 1){
